/**
 * PROMPTVAULT USA - CORE ENGINE v5.3
 * MODE: HEADLESS (Spreadsheet-Driven)
 * FIX: Cart Checkout Actually Renders + GMC Vault Links
 */
...
// --- 4. CART ENGINE (SMART BUNDLE) - FIXED ---
...
// NEW: Actually render the cart + trigger PayPal
window.renderCart = () => {
  ...
  // CRITICAL FIX: Actually render the PayPal button
...
// --- 5. NAVIGATION - FIXED TO TRIGGER CART ---
window.changePage = (id, el) => {
  ...
  if (id === 'cart') window.renderCart(); // CRITICAL FIX: Render cart when page opens
};
